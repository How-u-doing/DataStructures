...
public:
	...
	void has_cycle(std::vector<size_t>& cycle) {
		...
		std::vector<size_t> edge_to(N, UINT_MAX); // record the cycle(s)
		for (size_t i = 0; i < N; ++i)
			if (!marked[i])
				DFS_hc(i, i, marked, edge_to, cycle);
	}
private:
	void DFS_hc(size_t s, size_t u, std::vector<bool>& marked,
		std::vector<size_t>& edge_to, std::vector<size_t>& cycle)
	{
		marked[s] = true;
		for (const auto& e : _adj[s]) {
			//if (!cycle.empty()) return; // to find only one cycle
			size_t w = e._dest;
			if (!marked[w]) {	// u->s, s->w
				edge_to[w] = s; DFS_hc(w, s, marked, edge_to, cycle);
			}
			else if (w != u) {// found a cycle
				auto offset{ cycle.size() }; // starting position of this cycle
								
				for (size_t x = s; x != w; x = edge_to[x])
					if (edge_to[x] == UINT_MAX)
						return;
				for (size_t x = s; x != w; x = edge_to[x]) {
					//if (edge_to[x] == UINT_MAX) { // to erase those added vertices of this cycle
					//	cycle.erase(cycle.begin() + offset, cycle.end());
					//	return;
					//}
					cycle.push_back(x);
				}					
