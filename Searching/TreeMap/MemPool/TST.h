 ...
 *  https://github.com/How-u-doing/DataStructures/tree/master/Searching/TreeMap/MemPool/TST.h
 */
#ifndef TST_H
#define TST_H
#include "QtMemPool.h"
...
class TST {
	...
	struct Node {
		...
		~Node() { /*delete pval;*/ t_pool.recycle(this); }
		...
		static void operator delete(void* ptr) {
			return node_pool.recycle(ptr);
		}
	};

	inline static PoolAllocator<Node> node_pool; // requires c++17 or newer
	inline static PoolAllocator<T> t_pool;
		...
		for (;;) {
			...
			if		(key[d] < (*curr)->ch) { curr = & (*curr)->left;  pos = Link::LEFT; }
				...
				if ((*curr)->pval == nullptr) {
					(*curr)->pval = new (t_pool.allocate()) T(val); ++n;
				} // else do nothing
				return *curr;
			}
		}
	...
	void erase(node_ptr& x, const std::string& key, size_t d) {
		...
		if		(key[d] < x->ch) erase(x->left,  key, d);
			...
			if (x->pval != nullptr) { // found
				x->pval->~T(); t_pool.recycle(x->pval); --n;
				x->pval = nullptr;
			}
		}
