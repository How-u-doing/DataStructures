...
using namespace std;
using namespace mySortingAlgo;

int main()
{    
    array<int, 20> s1 = { 18,7,12,8,9,5,13,1,19,16,4,11,0,17,15,3,6,2,14,10 };
    int n1 = 10;
    quickselect(s1.begin(), s1.begin() + n1, s1.end()/*, greater<int>{}*/);
    for (int a : s1) {
        cout << a << " ";
    }
    ...
    int n2 = 4;
    partial_quicksort(s2.begin(), s2.begin() + n2, s2.end()/*, greater<int>{}*/);
    for (int a : s2) {
        cout << a << " ";
    }
    ...
    int n3 = 5;
    partial_sort(s3.begin(), s3.begin() + n3, s3.end(), greater<int>{});
    for (int a : s3) {
        cout << a << " ";
    }
